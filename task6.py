...
if __name__ == '__main__':
    ...
    trainloader = DataLoader(trainset, batch_size=32, shuffle=True)  # Adjusted batch size to 32
    testloader = DataLoader(testset, batch_size=1, shuffle=False)

    # Create the model and optimizer
    model = mobilenet_v3_small(weights=None, num_classes=2).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001)  # Adjusted learning rate to 0.001
